...
function imageUpload(): { payload: Buffer; contentType: string } {
  ...
  const footer = `\r\n--${boundary}--\r\n`;
  return {
    payload: Buffer.concat([Buffer.from(header), image, Buffer.from(footer)]),
...
test("business endpoints enforce authentication and standard validation", async () => {
  ...
  try {
    ...
    const invalid = await app.inject({
      ...
      url: "/api/v1/meals",
      headers: user("user-a"),
      payload: { mealType: "INVALID", capturedAt: "not-a-date", items: [] },
    });
...
test("meal CRUD is user-scoped and list responses are paginated", async () => {
  ...
  try {
    ...
    const own = await app.inject({
      method: "GET",
      url: "/api/v1/meals?page=1&pageSize=20",
      headers: user("user-a"),
    });
    assert.equal(own.statusCode, 200);
    assert.equal(own.json().data.length, 1);
    assert.deepEqual(own.json().meta, {
      page: 1,
      pageSize: 20,
      total: 1,
      totalPages: 1,
  ...
  try {
    ...
    const analysis = await app.inject({
      ...
      payload: {
        mealId,
        inputType: "IMAGE",
        inputReference: uploaded.json().data.reference,
      },
    });
    ...
    const pending = await app.inject({
      method: "GET",
      url: `/api/v1/meal-analysis/${analysisId}`,
      headers: user("user-a"),
    });
    ...
    const daily = await app.inject({
      method: "GET",
      url: "/api/v1/nutrition/daily?date=2026-08-25",
      headers: user("user-a"),
    });
    ...
    const water = await app.inject({
      ...
      url: "/api/v1/water",
      headers: user("user-a"),
      payload: { amountMl: 500, loggedAt: "2026-08-25T05:00:00.000Z" },
    });
    ...
    assert.deepEqual(chat.json().data.context, [
      "confirmed_meals",
      "daily_nutrition",
    ]);
    const barcode = await app.inject({
      method: "POST",
      url: "/api/v1/barcode/scan",
      headers: user("user-a"),
      payload: { barcode: "012345678901" },
    });
