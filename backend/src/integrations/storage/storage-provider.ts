...
export interface StorageProvider {
  ...
  getUrl(reference: StorageObjectReference, expiresInSeconds: number): Promise<string>;
...
function validateProviderKey(key: string, ownerId: string): void {
  ...
  if (!key.startsWith(expectedPrefix) || key.includes("..") || key.includes("\\") || key.includes("//")) {
...
export class LocalStorageProvider implements StorageProvider {
  ...
  async upload(input: StorageUploadInput): Promise<StoredObjectMetadata> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "EEXIST") {
        throw new AppError("STORAGE_ERROR", "Storage object already exists", { cause: error });
      }
      throw new AppError("STORAGE_ERROR", "Storage upload failed", { cause: error });
    }
    return {
      key: input.key,
  ...
  async getUrl(): Promise<string> {
    throw new AppError("STORAGE_ERROR", "Local storage uses authenticated application reads");
  }

  async download(reference: StorageObjectReference): Promise<StoredObjectContent> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        throw new AppError("STORAGE_OBJECT_NOT_FOUND", "Storage object was not found", { cause: error });
      }
  ...
  async delete(reference: StorageObjectReference): Promise<void> {
    ...
    try {
      ...
      throw new AppError("STORAGE_ERROR", "Storage deletion failed", { cause: error });
    }
  }
...
export class S3StorageProvider implements StorageProvider {
  ...
  async upload(input: StorageUploadInput): Promise<StoredObjectMetadata> {
    ...
    if (!response.ok) throw new AppError("STORAGE_ERROR", "Storage upload failed");
    return {
      key: input.key,
      ownerId: input.ownerId,
      contentType: input.contentType,
  ...
  async getUrl(reference: StorageObjectReference, expiresInSeconds: number): Promise<string> {
    validateProviderKey(reference.key, reference.ownerId);
    ...
    const signingSecret = deriveSigningSecret(this.options.secretKey, date, this.options.region);
    const stringToSign = `AWS4-HMAC-SHA256\n${amzDate}\n${date}/${this.options.region}/s3/aws4_request\n${sha256(canonicalRequest)}`;
    const signed = createHmac("sha256", signingSecret).update(stringToSign).digest("hex");
  ...
  async download(reference: StorageObjectReference): Promise<StoredObjectContent> {
    ...
    if (response.status === 404) throw new AppError("STORAGE_OBJECT_NOT_FOUND", "Storage object was not found");
    if (!response.ok) throw new AppError("STORAGE_ERROR", "Storage read failed");
  ...
  async delete(reference: StorageObjectReference): Promise<void> {
    ...
    if (!response.ok && response.status !== 404) throw new AppError("STORAGE_ERROR", "Storage deletion failed");
  }

  private async request(method: string, key: string, body?: Uint8Array, headers: Record<string, string> = {}): Promise<Response> {
    ...
    const amzDate = formatAmzDate(now);
    const date = amzDate.slice(0, 8);
    const canonicalHeaders: Record<string, string> = { ...headers, host, "x-amz-content-sha256": payloadHash, "x-amz-date": amzDate };
    const signedHeaders = Object.keys(canonicalHeaders).map((value) => value.toLowerCase()).sort();
    const canonicalHeaderString = signedHeaders.map((name) => `${name}:${canonicalHeaders[name]!.trim()}\n`).join("");
    const canonicalRequest = [method, path, "", canonicalHeaderString, signedHeaders.join(";"), payloadHash].join("\n");
    const scope = `${date}/${this.options.region}/s3/aws4_request`;
    const stringToSign = `AWS4-HMAC-SHA256\n${amzDate}\n${scope}\n${sha256(canonicalRequest)}`;
    const signature = createHmac("sha256", deriveSigningSecret(this.options.secretKey, date, this.options.region)).update(stringToSign).digest("hex");
    ...
    return fetch(`${this.baseUrl()}${path}`, { method, headers: { ...canonicalHeaders, authorization }, body: body ? Buffer.from(body) : undefined });
  }

  private objectUrl(key: string): { host: string; path: string } {
    const endpoint = new URL(this.options.endpoint ?? `https://s3.${this.options.region}.amazonaws.com`);
    const path = `/${this.options.bucket}/${key}`;
    return { host: endpoint.host, path: `${endpoint.pathname.replace(/\/$/, "")}${path}`.replace(/\/+/g, "/") };
  }
...
function deriveSigningSecret(secret: string, date: string, region: string): Buffer {
