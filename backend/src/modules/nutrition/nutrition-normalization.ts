...
export type NutritionMappingType =
  | "EXACT_MATCH"
  | "CLOSE_MATCH"
  | "DERIVED_ESTIMATE"
  | "ESTIMATED"
...
const MAPPING_TYPES = new Set<NutritionMappingType>([
  "EXACT_MATCH",
  "CLOSE_MATCH",
  "DERIVED_ESTIMATE",
  "ESTIMATED",
...
export function convertUnit(
  ...
  if (!Number.isFinite(value))
    throw new Error("Nutrition value must be finite");
  if (from === to) return value;
  if (MASS_TO_GRAMS[from] !== undefined && MASS_TO_GRAMS[to] !== undefined)
...
export function energyToKcal(value: number, unit: "kcal" | "kJ"): number {
  if (!Number.isFinite(value) || value < 0)
    throw new Error("Energy must be a non-negative finite number");
  return unit === "kJ" ? value / 4.184 : value;
}

export function scaleNutrition<T extends NutritionValues>(
  values: T,
  requestedQuantity: number,
  referenceQuantity: number,
): T {
  if (!Number.isFinite(requestedQuantity) || requestedQuantity <= 0)
    throw new Error("Requested serving quantity must be positive");
  if (!Number.isFinite(referenceQuantity) || referenceQuantity <= 0)
    throw new Error("Reference quantity must be positive");
  const factor = requestedQuantity / referenceQuantity;
  return Object.fromEntries(
    VALUE_FIELDS.map((field) => [
      field,
      values[field] === null ? null : values[field] * factor,
    ]),
  ) as unknown as T;
}
...
export function normalizeNutrition(
  ...
  if (
    ...
    throw new Error("Nutrition provenance is required");
  if (
    !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(input.sourceId) ||
    !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(input.sourceVersion)
  )
    throw new Error("Malformed nutrition source identifier");
  if (!Number.isFinite(input.referenceQuantity) || input.referenceQuantity <= 0)
    throw new Error("Reference quantity must be positive");
  if (!(["g", "ml", "serving"] as const).includes(input.referenceUnit))
    throw new Error("Invalid reference unit");
  if (input.mappingType !== undefined && !MAPPING_TYPES.has(input.mappingType))
    throw new Error("Invalid nutrition mapping type");
  const basis: ReferenceBasis =
    input.referenceUnit === "g" && input.referenceQuantity === 100
      ? "PER_100_G"
      : input.referenceUnit === "ml" && input.referenceQuantity === 100
        ? "PER_100_ML"
        : "PER_SERVING";
  ...
  for (const field of VALUE_FIELDS) {
    ...
    const unit =
      input.units?.[field] ??
      (field === "calories" ? "kcal" : field === "sodium" ? "mg" : "g");
    if (field === "calories") {
      if (unit !== "kcal" && unit !== "kJ")
        throw new Error("Energy unit must be kcal or kJ");
  ...
  const macroEnergy =
    values.protein === null ||
    values.carbohydrates === null ||
    values.fat === null
      ? null
      : values.protein * 4 + values.carbohydrates * 4 + values.fat * 9;
  const consistencyWarning =
    values.calories !== null &&
    macroEnergy !== null &&
    Math.abs(values.calories - macroEnergy) >
      Math.max(25, values.calories * 0.25);
  return {
    ...values,
    foodId: input.foodId,
    ...
    mappingType: input.mappingType ?? "EXACT_MATCH",
