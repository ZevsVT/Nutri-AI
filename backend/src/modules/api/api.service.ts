...
interface ScaledNutrition extends NutritionTotals {
  sugar: number;
  sodium: number;
}
...
export interface BusinessApiService {
  ...
  ): Promise<{ data: MealDto[]; total: number }>;
  ...
  createAnalysis(
    userId: string,
    input: { mealId?: string; inputType: string; inputReference: string },
  ): Promise<AnalysisDto>;
  ...
  createWater(
    userId: string,
    input: { amountMl: number; loggedAt: Date },
  ): Promise<Record<string, unknown>>;
  listWater(
    userId: string,
    input: { from?: Date; to?: Date; page: number; pageSize: number },
  ): Promise<{ data: Record<string, unknown>[]; total: number }>;
  listInsights(
    userId: string,
...
interface MemoryAnalysis extends AnalysisDto {
  userId: string;
  corrections?: MealItemInput[];
}
...
export class InMemoryBusinessApiService implements BusinessApiService {
  constructor(
    private readonly barcodeProvider: BarcodeProvider = new NoopBarcodeProvider(),
    private readonly storageService?: StorageService,
  ) {}
  private readonly foods: MemoryFood[] = [
    {
      id: "food-demo",
  ...
  private readonly water: Array<
    Record<string, unknown> & { userId: string; loggedAt: Date }
  > = [];
  ...
  async createMeal(userId: string, input: MealInput) {
    ...
    const meal: MemoryMeal = {
      id: randomUUID(),
      userId,
      mealType: input.mealType,
      name: input.name ?? "Untitled meal",
      ...
      items,
      totals: items.reduce(
        (sum, item) => addTotals(sum, item.nutrition ?? emptyTotals()),
        emptyTotals(),
      ),
      deleted: false,
    };
  ...
  async listMeals(
    ...
    const rows = this.meals.filter(
      (meal) =>
        !meal.deleted &&
        meal.userId === userId &&
        (!input.from || new Date(meal.capturedAt) >= input.from) &&
        (!input.to || new Date(meal.capturedAt) <= input.to) &&
        (!input.mealType || meal.mealType === input.mealType) &&
        (!input.status || meal.status === input.status),
    );
    rows.sort((a, b) => b.capturedAt.localeCompare(a.capturedAt));
    return {
      data: pageOf(
        rows.map((meal) => this.publicMeal(meal)),
        input.page,
        input.pageSize,
      ),
      total: rows.length,
  ...
  async updateMeal(
    ...
    const meal = this.findMeal(userId, id);
    if (meal.status === "CONFIRMED" && (input.mealType || input.capturedAt))
      throw new AppError(
        "INVALID_STATE",
        "Confirmed meal nutrition cannot be changed",
      );
    Object.assign(
      meal,
  ...
  async deleteMeal(userId: string, id: string) {
    const meal = this.findMeal(userId, id);
    meal.deleted = true;
    await this.storageService?.retainAfterMealDelete(
      meal.imageReference,
      userId,
    );
  }
  ...
  async createAnalysis(
    ...
    if (input.inputType === "IMAGE") {
      ...
      await this.storageService.getByReferenceForUser(
    ...
    if (input.inputType === "IMAGE")
      await this.storageService?.attachForUser(input.inputReference, userId);
    return this.publicAnalysis(analysis);
  }

  async getAnalysis(userId: string, id: string) {
    return this.publicAnalysis(this.findAnalysis(userId, id));
  }

  async confirmAnalysis(userId: string, id: string, items: MealItemInput[]) {
    const analysis = this.findAnalysis(userId, id);
    if (analysis.status === "FAILED")
      throw new AppError("ANALYSIS_FAILED", "Meal analysis failed");
    if (analysis.status === "PENDING" || analysis.status === "PROCESSING")
      throw new AppError("ANALYSIS_NOT_READY", "Meal analysis is not ready");
    const meal = this.findMeal(userId, analysis.mealId);
    const mapped = items.map((item) => this.toMealItem(item));
    meal.items = mapped;
    meal.status = "CONFIRMED";
    meal.confirmedAt = new Date().toISOString();
    meal.totals = mapped.reduce(
      (sum, item) => addTotals(sum, item.nutrition ?? emptyTotals()),
      emptyTotals(),
    );
  ...
  async dailyNutrition(userId: string, date: Date) {
    ...
    const meals = this.meals.filter(
      (meal) =>
        ...
        !meal.deleted &&
        meal.status === "CONFIRMED" &&
        meal.capturedAt.slice(0, 10) === day,
    );
    return this.summary(meals);
  }
  async weeklyNutrition(userId: string, from: Date, to: Date) {
    const meals = this.meals.filter(
      (meal) =>
        meal.userId === userId &&
        !meal.deleted &&
        meal.status === "CONFIRMED" &&
        new Date(meal.capturedAt) >= from &&
        new Date(meal.capturedAt) <= to,
    );
    return this.summary(meals);
  }
  async chat(userId: string, message: string) {
    const summary = await this.dailyNutrition(userId, new Date());
    return {
      message: `Based on your confirmed meals, ${message.trim()} — today’s recorded calories are ${summary.totals.calories}.`,
      context: ["confirmed_meals", "daily_nutrition"],
    };
  }
  ...
  async createWater(
    ...
    this.water.push({ ...row, loggedAt: input.loggedAt });
    return row;
  }
  async listWater(
    userId: string,
    ...
    const rows = this.water.filter(
      (row) =>
        row.userId === userId &&
        (!input.from || row.loggedAt >= input.from) &&
        (!input.to || row.loggedAt <= input.to),
    );
    return {
      data: pageOf(rows, input.page, input.pageSize),
      total: rows.length,
    };
  }
  async listInsights(
  ...
  async scanBarcode(_userId: string, barcode: string) {
    const product = await this.barcodeProvider.lookup(barcode);
    return {
      barcode,
      status: product ? "RESOLVED" : "UNRESOLVED",
      product,
      source: product?.source ?? null,
    };
  }

  private findMeal(userId: string, id: string) {
  ...
  private findAnalysis(userId: string, id: string) {
    const analysis = this.analyses.find(
      (candidate) => candidate.analysisId === id && candidate.userId === userId,
    );
    if (!analysis)
      throw new AppError("ANALYSIS_NOT_FOUND", "Meal analysis was not found");
    return analysis;
  }
  private toMealItem(item: MealItemInput) {
    if (item.quantity <= 0)
      throw new AppError("VALIDATION_ERROR", "Quantity must be positive");
    const food = this.foods.find((candidate) => candidate.id === item.foodId);
    if (!food) throw new AppError("FOOD_NOT_FOUND", "Food was not found");
    return {
      ...item,
      id: randomUUID(),
      displayName: item.displayName ?? food.name.vi,
      nutrition: emptyTotals(),
    };
  }
  private publicFood(food: MemoryFood, includeNutrition = false): FoodDto {
    return {
      id: food.id,
      name: food.name,
      category: food.category,
      cuisine: food.cuisine,
      ...(includeNutrition ? { nutrition: { ...food.nutrition } } : {}),
    };
  }
  private publicMeal(meal: MemoryMeal): MealDto {
    const { userId, deleted, ...publicMeal } = meal;
    void userId;
    void deleted;
    return publicMeal;
  }
  private publicAnalysis(analysis: MemoryAnalysis): AnalysisDto {
    const { userId, corrections, ...publicAnalysis } = analysis;
    void userId;
    void corrections;
    return publicAnalysis;
  }
  private summary(meals: MemoryMeal[]) {
    return {
      period: {
        from: meals.at(-1)?.capturedAt ?? null,
        to: meals[0]?.capturedAt ?? null,
      },
      totals: meals.reduce(
        (sum, meal) => addTotals(sum, meal.totals),
        emptyTotals(),
      ),
      mealCount: meals.length,
      meals: meals.map((meal) => this.publicMeal(meal)),
    };
  }
}
...
export class PrismaBusinessApiService implements BusinessApiService {
  ...
  async createMeal(userId: string, input: MealInput) {
    const mealId = await this.prisma.$transaction(async (tx) => {
      const foods = await tx.food.findMany({
        where: {
          id: { in: input.items.map((item) => item.foodId) },
          isActive: true,
        },
        include: {
          nutritionRecords: {
      ...
      if (foods.length !== new Set(input.items.map((item) => item.foodId)).size)
        throw new AppError(
          "FOOD_NOT_FOUND",
          "One or more foods were not found",
        );
      ...
      for (const item of input.items) {
        ...
        if (!food || !nutrition)
          throw new AppError(
            ...
            "Nutrition data is unavailable for one or more foods",
          );
        const itemNutrition = this.scaledNutrition(nutrition, item.quantity);
        Object.assign(totals, addTotals(totals, itemNutrition));
        const createdItem = await tx.mealItem.create({
          data: {
            mealId: meal.id,
            foodId: food.id,
            quantity: item.quantity,
            unit: item.unit,
            displayName: item.displayName ?? food.nameVi,
          },
        });
        await tx.mealItemNutrition.create({
          data: {
            mealItemId: createdItem.id,
            nutritionVersionId: nutrition.nutritionVersionId,
            sourceId: nutrition.nutritionVersion.sourceId,
            servingAmount: item.quantity,
      ...
      await tx.nutritionAnalysis.create({
        data: {
          mealId: meal.id,
          status: "COMPLETED",
  ...
  async listMeals(
    ...
    const where: Prisma.MealWhereInput = {
      ...
      ...(input.status
        ? { status: input.status as Prisma.MealWhereInput["status"] }
        : {}),
      capturedAt: {
  ...
  async updateMeal(
    ...
    if (existing.status === "CONFIRMED" && (input.mealType || input.capturedAt))
      throw new AppError(
        "INVALID_STATE",
        "Confirmed meal nutrition cannot be changed",
      );
  ...
  async deleteMeal(userId: string, id: string) {
    ...
    if (result.count === 0)
      throw new AppError("MEAL_NOT_FOUND", "Meal was not found");
    await this.storageService?.retainAfterMealDelete(meal.imageUrl, userId);
  }
  ...
  async createAnalysis(
    ...
    if (mealId) {
      ...
      if (input.inputType === "IMAGE")
        await this.prisma.meal.update({
          where: { id: mealId },
          data: { imageUrl: input.inputReference, status: "ANALYZING" },
        });
    } else
      mealId = (
    ...
    const analysis = await this.prisma.aIAnalysis.create({
      data: {
        ...
        model: "pending",
        inputType: input.inputType as "IMAGE" | "TEXT" | "BARCODE" | "MANUAL",
        inputReference: input.inputReference,
      },
    });
    if (input.inputType === "IMAGE")
  ...
  async confirmAnalysis(userId: string, id: string, items: MealItemInput[]) {
    const mealId = await this.prisma.$transaction(async (tx) => {
      const analysis = await tx.aIAnalysis.findFirst({
        where: { id, userId },
        select: { id: true, mealId: true, status: true },
      });
      if (!analysis)
        throw new AppError("ANALYSIS_NOT_FOUND", "Meal analysis was not found");
      if (analysis.status === "FAILED")
      ...
      if (foods.length !== new Set(items.map((item) => item.foodId)).size)
        throw new AppError(
          "FOOD_NOT_FOUND",
          "One or more foods were not found",
        );
      ...
      for (const item of items) {
        ...
        const values = this.scaledNutrition(nutrition, item.quantity);
        Object.assign(totals, addTotals(totals, values));
      ...
      await tx.nutritionAnalysis.upsert({
  ...
  async dailyNutrition(userId: string, date: Date) {
    ...
    return this.aggregateNutrition(userId, from, to);
  }
  async weeklyNutrition(userId: string, from: Date, to: Date) {
    return this.aggregateNutrition(userId, from, to);
  }
  ...
  async listRecipes(input: {
    ...
    const { cuisine, page, pageSize } = input;
    void input.tags;
    const where: Prisma.RecipeWhereInput = {
      ...
      ...(cuisine
        ? {
            OR: [
              { nameVi: { contains: cuisine, mode: "insensitive" } },
              { nameEn: { contains: cuisine, mode: "insensitive" } },
            ],
          }
        : {}),
    };
    ...
    return {
      data: rows.map((recipe) => ({
        id: recipe.id,
        name: { vi: recipe.nameVi, en: recipe.nameEn },
  ...
  async createWater(
    ...
    return this.prisma.waterLog.create({
      data: { userId, amountMl: input.amountMl, loggedAt: input.loggedAt },
  ...
  async listWater(
    ...
    const where: Prisma.WaterLogWhereInput = {
      userId,
      loggedAt: {
        ...(input.from ? { gte: input.from } : {}),
        ...(input.to ? { lte: input.to } : {}),
  ...
  async listInsights(
    ...
    const [rows, total] = await this.prisma.$transaction([
      ...
      this.prisma.nutritionInsight.count({ where }),
    ]);
    return { data: rows, total };
  }
  async scanBarcode(_userId: string, barcode: string) {
  ...
  private foodDto(
    food: {
      ...
      cookingMethod?: string | null;
      servingUnit?: string | null;
      defaultServingSize?: unknown;
      nutritionRecords?: readonly NutritionRecordLike[];
    },
    ...
    const dto: FoodDto = {
      id: food.id,
      slug: food.slug,
      name: { vi: food.nameVi, en: food.nameEn },
      category: food.category,
      cuisine: food.cuisine,
      ...
      defaultServingSize:
        food.defaultServingSize == null
          ? null
          : Number(food.defaultServingSize),
    };
    if (includeNutrition && food.nutritionRecords?.length) {
      const record = this.latestNutrition(food.nutritionRecords);
      if (record)
        dto.nutrition = {
          ...
          carbohydrates: Number(record.carbohydrates),
          fat: Number(record.fat),
          fiber: Number(record.fiber),
          source: record.nutritionVersion.source.name,
          provider: record.nutritionVersion.source.provider,
          version: record.nutritionVersion.version,
        };
    }
    return dto;
  }
  private latestNutrition(records: readonly NutritionRecordLike[]) {
    return [...records].sort(
      (a, b) =>
        new Date(b.nutritionVersion.effectiveFrom).getTime() -
        new Date(a.nutritionVersion.effectiveFrom).getTime(),
    )[0];
  }
  private scaledNutrition(
    record: NutritionRecordLike,
    quantity: number,
  ): ScaledNutrition {
    const factor = quantity / Number(record.servingAmount);
    return {
      calories: Number(record.calories) * factor,
      protein: Number(record.protein) * factor,
      carbohydrates: Number(record.carbohydrates) * factor,
      fat: Number(record.fat) * factor,
      fiber: Number(record.fiber) * factor,
      sugar: Number(record.sugar) * factor,
      sodium: Number(record.sodium) * factor,
    };
  }
  private mealDto(meal: MealLike): MealDto {
    const items = meal.items.map((item) => ({
      ...
      nutrition: item.nutritionSnapshot
        ? {
            calories: Number(item.nutritionSnapshot.calories),
            protein: Number(item.nutritionSnapshot.protein),
            carbohydrates: Number(item.nutritionSnapshot.carbohydrates),
            fat: Number(item.nutritionSnapshot.fat),
            fiber: Number(item.nutritionSnapshot.fiber),
          }
        : undefined,
    }));
    return {
      ...
      name: meal.name,
      capturedAt: meal.capturedAt.toISOString(),
      status: meal.status,
      notes: meal.notes,
      confirmedAt: meal.confirmedAt?.toISOString() ?? null,
      imageReference: meal.imageUrl ?? null,
      items,
      totals: items.reduce(
        (sum: NutritionTotals, item) =>
          addTotals(sum, item.nutrition ?? emptyTotals()),
        emptyTotals(),
      ),
    };
  }
      ...
      status:
        analysis.status === "RUNNING"
          ...
          : analysis.status === "COMPLETED"
            ? "READY"
            : (analysis.status as AnalysisDto["status"]),
      inputType: analysis.inputType,
      inputReference: analysis.inputReference ?? null,
      predictions: predictions.map((prediction) => ({
        id: prediction.id,
        predictedName: prediction.predictedName,
        foodId: prediction.foodId,
        quantity:
          prediction.estimatedQuantity === null
            ? null
            : Number(prediction.estimatedQuantity),
        unit: prediction.estimatedUnit,
        confidence:
          prediction.confidence === null ? null : Number(prediction.confidence),
      })),
    };
  }
  private async aggregateNutrition(userId: string, from: Date, to: Date) {
    const meals = await this.prisma.meal.findMany({
      ...
      include: { items: { include: { nutritionSnapshot: true } } },
    ...
    return {
      ...
      totals,
      mealCount: meals.length,
      meals: meals.map((meal) => this.mealDto(meal)),
    };
  }
}
